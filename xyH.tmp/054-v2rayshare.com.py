...
def main():
    ...
    LastestClashNodeLink=soup.find('a', title=re.compile("日」\d{4}年最新高速"))
    ...
    html_content = response.content
    soup = BeautifulSoup(html_content, 'html.parser')
    tagYaml=soup.find('p', string=re.compile("v2rayshare.githubrowcontent.com/\d{4}/\d{2}/\d{4}\d{2}\d{2}.yaml"))
    if not tagYaml:
