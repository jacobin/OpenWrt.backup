...
for depend in "${depends[@]}"; do
    opkg install ${depend} --force-depends > NUL 2>&1
    if [ $? -ne 0 ]; then
        echo "failed: ${depend}"
    fi
done
 ...
 "/etc/init.d/openclash" stop
opkg install "./luci-app-openclash_0.47.028_all.ipk" --force-depends > NUL 2>&1
if [ $? -ne 0 ]; then
    echo "failed: \"./luci-app-openclash_0.47.028_all.ipk\""
    exit 1
fi
