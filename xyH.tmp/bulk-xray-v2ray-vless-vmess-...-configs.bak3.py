...
def get_v2ray_links(session, name, url):
    try:
        ...
        if response.status_code == 200:
            ...
            now = datetime.now()
            time_ago = now - timedelta(days=300, hours=30, minutes=15)
            time_ago2 = time_ago.astimezone(ZoneInfo("UTC"))

            start_tag = soup.find(lambda tag: \
                                        tag.name == 'time' \
                                        and tag.has_attr('datetime') and time_ago2 < datetime.fromisoformat(tag['datetime']) \
                                        )

            if start_tag:
                if start_tag.parent:
                    if start_tag.parent.parent:
                        if start_tag.parent.parent.parent:
                            if start_tag.parent.parent.parent.parent:
                                pppp=start_tag.parent.parent.parent.parent

            if pppp:
                tags_newer = []
                current_element = pppp.next_sibling
                while current_element:
                    # Check if the element is an actual tag and not just a newline or whitespace
                    if isinstance(current_element, Tag):
                        tags_newer.append(current_element)
                    ...
                    #     tags_newer.append(current_element.strip())
                    current_element = current_element.next_sibling

            for tag in tags_newer:
                soup = BeautifulSoup(tag.content, 'html.parser')
...
if __name__ == "__main__":
    ...
    for Channel in telegram_urls:
        ...
        url=parts[1].strip()
        print(f"{url}")
        v2ray_configs = get_v2ray_links(session, name, url)
        if v2ray_configs:
