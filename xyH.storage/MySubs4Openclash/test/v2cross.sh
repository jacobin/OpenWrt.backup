...
function fnExtractSubsCallback() {
    local usage="Usage: $(basename $BASH_SOURCE) <-uRefUrl> <-sOutSuburls> <-tThreadDatetime> <-h>"
   ...
   #local outSuburls
    ...
    cat "${tmpRefUrl}" | grep "https://9527521.xyz/pubconfig/" > "${tmpOnlysubs}"
    rm -f "${tmpRefUrl}"
