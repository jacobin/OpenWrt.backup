...
function combine_subscri() {
    ...
    for (( i = 0; i < ${groupCount}; i++ )); do
        ...
        echo -e "\toption enabled '1'"             >> ${fpathMuta}
        echo -e "\toption name '${resultOptName}'" >> ${fpathMuta}
        echo -e "\toption address '${CONVERTER}/sub?target=clashr"`
                `"&config=ACL4SSR_Online_Full_AdblockPlus.ini&emoji=true"`
                `"&list=false&udp=true&tfo=true&scv=true&fdn=true"`
                `"&enable_filter=true&append_type=true"`
                `"&filter_script=function%20filter%28N%29%7Bif%28N.Type%3D%3D"`
                `"%3D0%7C%7CN.Type%3D%3D%3D1%7C%7CN.Type%3D%3D%3D2%7C%7CN.Typ"`
                `"e%3D%3D%3D4%7C%7CN.Type%3D%3D%3D6%7C%7CN.Type%3D%3D%3D7%7C%"`
                `"7CN.Type%3D%3D%3D8%29%7Breturn%20true%3B%7Dlet%20M%3DN.Encr"`
                `"yptMethod%3Bif%28M%3D%3D%3Dnull%7C%7CM.length%3D%3D%3D0%29%"`
                `"7Bif%28N.Type%3D%3D%3D1%29%7Breturn%20true%3B%7Dreturn%20fa"`
                `"lse%3B%7Dlet%20C%3D%5B%27aes-128-cfb%27%2C%27aes-128-ctr%27"`
                `"%2C%27aes-128-gcm%27%2C%27aes-192-cfb%27%2C%27aes-192-ctr%2"`
                `"7%2C%27aes-192-gcm%27%2C%27aes-256-cfb%27%2C%27aes-256-ctr%"`
                `"27%2C%27aes-256-gcm%27%2C%27auto%27%2C%27chacha20%27%2C%27c"`
                `"hacha20-ietf%27%2C%27chacha20-ietf-poly1305%27%2C%27rc4-md5"`
                `"%27%2C%27xchacha20%27%2C%27xchacha20-ietf-poly1305%27%5D%3B"`
                `"let%20m%3DM.toLowerCase%28%29%3Bfor%28let%20i%3D0%3Bi%3CC.l"`
                `"ength%3Bi%2B%2B%29%7Bif%28m%3D%3D%3DC%5Bi%5D%29%7Breturn%20"`
                `"false%3B%7D%7Dreturn%20true%3B%7D&exclude=%28CN%7CHK%7CHong"`
                `"%20Kong%7CHongKong%7Cv2cross%7CHONG%20KONG%7CHONGKONG%7CV2C"`
                `"ROSS%7CHongkong%7C%E5%BB%A3%E6%9D%B1%7C%E5%8C%97%E4%BA%AC%7"`
                `"C%E5%B9%BF%E4%B8%9C%7C%E8%B4%B5%E5%B7%9E%7C%E4%B8%8A%E6%B5%"`
                `"B7%7C%E9%A6%99%E6%B8%AF%7C%E7%A7%BB%E5%8B%95%7C%E7%A7%BB%E5"`
                `"%8A%A8%7C%E4%B8%AD%E5%9C%8B%7C%E4%B8%AD%E5%9B%BD%7C%E8%B2%B"`
                `"4%E5%B7%9E%7C%E5%85%8D%E8%B4%B9%7C%E8%AE%A2%E9%98%85%7C%E8%"`
                `"AE%A2%E9%98%85%E9%9A%8F%E6%97%B6%E4%BC%9A%E5%A4%B1%E6%95%88"`
                `"%7C%E6%97%A5%E6%9C%9F%7C%E7%94%B5%E6%8A%A5%7C%E7%94%B5%E6%8"`
                `"A%A5%E7%BE%A4%7CHTTP%7CHTTPS%7CSOCKS5%29&url=${url_uhttpd}'"\
                   >> ${fpathMuta}
        echo -e "" >> ${fpathMuta}
    done
...
#                          /\       |                                                              |                       ----> / LinkWorthTrying  /
#    /--------------/     /  \      |    |------------------------|      |--------------------|    |    |-------------|    |    /------------------/
#   / SubscriUrls  /---->/ O7 \---->|--->|      LinkInactive      | ---> | LinkInactiveOver7  | ---|--> | LinkDiscard | ---|
#  /--------------/      \    /     |    |------------------------|      |--------------------|    |    |-------------|    |      /---------------------/
#                         \  /      |                                                              |                       ----> / LinkNotWorthTrying  /
#                          \/       |    |------------------------|      |--------------------|    |                            /---------------------/
...
############################# function: _DEBUG ################################
###############################################################################
# # Debugging switch, on means enabled, others mean disabled
# # IS_DEBUG="off"
# # Debugging switch function
function _DEBUG() {
    [ "$IS_DEBUG" == "on" ] && $@
}
# va=1
...
## fnStacktrace ###############################################################
###############################################################################
function fnStacktrace() {
    local traceinfo=
    local i pref="TRACE: "
    for (( i=${#FUNCNAME[@]}-1; i; i-- )); do
        traceinfo="${traceinfo}$pref${FUNCNAME[i]}[${BASH_LINENO[i-1]}]"
        pref=" -> "
    done
    tee_echo "${traceinfo}: $1"
}

# ###########################################################################
# ## fnStacktrace2 ##########################################################
# ###########################################################################
# function fnStacktrace2 {
...
#     count=-1
#     echo case 1 ++++++++++++++++++++
#     (( count > 0 )) || fnStacktrace "Unexpected count: $count"
#
#     echo -e "\ncase 2 ++++++++++++++++++++"
#     (( count > 0 )) || fnStacktrace2
# }
#
# #//////////////////////////////////////////////////////////////////////////
# function func_b() {
