...
def main():
    ...
    try:
        ...
        for currentArg, currentVal in arguments:
            if currentArg in ("-h", "--help"):
                print( "python [-h/--help] [-i/--input inputFPath] [-o/--output outputFPath]" )
                return 0
            elif currentArg in ("-i", "--input"):
                inputYamls.append(currentVal)
            elif currentArg in ("-o", "--output"):
                outputV2ray=currentVal
    ...
    for inputYaml in inputYamls:
        config = load_yaml_config(inputYaml)
        links = convert_proxies(config)
        all_links.extend(links)

    with open(outputV2ray, 'w', encoding='utf-8') as f:
        f.write('\n'.join(all_links))
